...
Grapple.prototype.pull =  function(body1,body2){
	if (true){ //show mass and speed of what pulls what is detemined by mass difference
		...
		game.physics.arcade.moveToXY(body1, body2.worldX, body2.worldY, this.grappleStats.pullSpeed);
		body1.body.allowGravity = false;
		this.events.isCanceled = false;
...
Grapple.prototype.killGrapple = function(){
	console.log(this.events.isReturning);
	console.log(this);
	this.events.isReturning = false;
...
Grapple.prototype.update = function(){
	...
	if (this.grappleHook && !this.events.isReturning)
	{
		if (game.physics.arcade.collide(this.grappleHook, layer, function(){grapple.pull(grapple.attachedTo,grapple.grappleHook);}))
		{
			this.events.isReached = true;
			this.grappleHook.body.allowGravity = false;
			this.grappleHook.body.velocity.set(0);
			game.physics.arcade.moveToObject(this.attachedTo, this.grappleHook, this.grappleStats.pullSpeed);
	...
	if(this.events.isReturning)
	{
		...
		game.physics.arcade.overlap(this.grappleHook, this.attachedTo, function(){grapple.killGrapple();});

	}

	if(this.events.isReached && !this.events.isReturned && !this.events.isReturning)
	{
		game.physics.arcade.moveToObject(this.attachedTo, this.grappleHook, this.grappleStats.pullSpeed);
	}


	if (game.physics.arcade.overlap(this.grappleHook, this.attachedTo) && this.events.isReached)
	{
