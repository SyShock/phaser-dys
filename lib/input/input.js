...
function pointerDown(pointer){
	if (game.input.activePointer.withinGame){
		...
		if(pointer.leftButton.isUp ) {
			if(pointer.id !== pressedId && !player.events.grappleMode){
				if(eventTimer+selectedTime > game.time.now){
					player.events.grappleMode = true;
					grapple.startLine();
				}
				else{
					eventTimer = game.time.now;

					initialY = game.input.y;
					if (game.input.x < player.player.body.x-game.camera.x)
					{
						moveLeft(true);
					}
					else
...
function pointerUp(pointer){
	if (game.input.activePointer.withinGame)
	{
		...
		if(pointer.leftButton.isUp){
			if(game.input.activePointer.id !== pressedId){
