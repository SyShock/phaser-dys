var Unit = function (index, game, cachedSpriteName,type, startX, startY, hasPhysics ,scale, alpha, conscious, patrolLine, mass, friction, bounce){
	this.type = type;
	if(type === "drone")
		this.type = new HostileDrone(index, game, cachedSpriteName,type, startX, startY, hasPhysics, scale, alpha, conscious, patrolLine, mass, friction, bounce);
...
var HostileDrone =  function(index, game, cachedSpriteName,type, startX, startY, hasPhysics, scale, alpha, conscious, patrolLine , mass, friction, bounce){
	var x = startX;
	var y = startY;
	this.game = game;
	if (conscious) this.conscious = true;
	...
	if(hasPhysics){
		...
		this.ghost = new Player(0, game, 'player', player.player.body.x,
				player.player.body.y, null,false, 0.2, 0.2);
		this.patrol = new Unit(0,game,'drone','drone',x,y,false,0.4,0.2);
	}
};
...
HostileDrone.prototype.update = function(){
	...
	for (var i = 0; worldItems.length > i; i++){
		...
		if(seesPlayer){
				if (this.sightRange > game.physics.arcade.distanceBetween(this.unit,player.player)) 
					if ( game.physics.arcade.angleBetween(this.unit,player.player) < this.unit.rotation + this.coneOfVision &&
						game.physics.arcade.angleBetween(this.unit,player.player) > this.unit.rotation -  this.coneOfVision)
